...
**Indexer 服务 - 大块处理优化**
- 🛡️ **大块内存优化**：扫描超过可配置阈值（默认 50MB）的区块时，索引器不再将整块原始数据加载到内存。改为先通过 `getblock` verbosity=1 获取区块的 txid 列表，再按 txid 逐个调用 `getrawtransaction` 获取交易并逐笔处理，避免超大块（如 800MB+）导致内存激增甚至 OOM。
- 📊 **不跳过任何块**：所有区块均会参与索引；大块采用按交易逐笔的懒加载方式处理。
- ⚙️ **新增配置**：`indexer.large_block_size_mb` — 超过该大小（MB）的区块走懒加载路径；为 0 时使用默认 200, 即超过 200MB 的区块走懒加载路径。
